...
; (defsetf) - define setf method ; 2015 -- added comment field to two argument defsetf.
(defmacro defsetf (sym first &rest rest)
          (if (symbolp first)
              (progn
               (when rest
                     (setf (get sym '%doc-setf) (car rest)))
...
;; Neither of the above are correct! Tom Almy 2/2011
;; I also added a string-downcase since we generally don't want
;; uppercase filenames.

(defun require (name &rest pathlist)
       (if (find (string name) *modules* :test #'string=)
           t
           (if (null pathlist)
               (load (string-downcase (string name)))
               (reduce #'and (mapcar #'load pathlist)))))
...
; Improved by TAA again in 2011 to match revised common lisp definition
(defun functionp (x)
  (typep x '(or closure subr)))
